...
public partial class Lecturer_groupEmail : System.Web.UI.Page
{
    ...
    protected void send_Click(object sender, EventArgs e)
    {
        ...
        //JC - Array list to hold selected email ids
        ArrayList emailArray = new ArrayList();
        //JC - Looping through rows of GridView
        foreach (GridViewRow item in GridView1.Rows)
        {
            //JC - Creating checkbox object using the find control method
            CheckBox cb = (CheckBox)item.Cells[1].FindControl("CheckBox1");

            //JC - Checking whether checkbox is checked or not
            if (cb.Checked)
            {
                //JC - If checked Adding email id to Arraylist

                emailArray.Add(GridView1.DataKeys[item.RowIndex]["email"].ToString());
            }
        }
        //JC - Looping through the email id list
