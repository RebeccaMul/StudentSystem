<%@ Page Language="C#" AutoEventWireup="true" CodeFile="classList.aspx.cs" Inherits="Lecturer_classList" %>
...
<body class="nav-md">
  ...
  <div class="panel-body">
    Course
      ...
      <!-- RME sql data source for one table to pull data from mylittleadmin-->  <!-- Session used as filtering parameter-->
         ...
         SelectCommand="SELECT DISTINCT *,firstName +' '+ lastName AS name FROM SIS_STUDENT_MODULES
               INNER JOIN SIS_STUDENT 
               ON SIS_STUDENT.studentID = SIS_STUDENT_MODULES.studentID 
               INNER JOIN SIS_RESULTS
               ON SIS_STUDENT.studentID = SIS_RESULTS.studentID
               INNER JOIN SIS_ASSIGNMENT
               ON SIS_RESULTS.assignmentID = SIS_ASSIGNMENT.assignmentID
               WHERE SIS_STUDENT_MODULES.moduleSelected = @id
               AND SIS_ASSIGNMENT.assignmentName = 'Exam' 
               AND SIS_RESULTS.moduleID = @id 
               AND SIS_STUDENT_MODULES.Completed = 'False' ORDER BY SIS_STUDENT.lastName">
  ...
  <div class="panel-body">
    Course
      ...
      Result:
                            <asp:Label Text='<%# Eval("resultMark") %>' runat="server" ID="Label1" /><br />
                                <br />
