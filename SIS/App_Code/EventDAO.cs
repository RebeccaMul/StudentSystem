...
public class EventDAO
{
    ...
    private static string connectionString="Data Source=net.eeecs.qub.ac.uk; Persist Security Info=True; User ID=etroyo01; Initial Catalog=etroyo01; Password=x56y050kwm6bsmh4";
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE SIS_EVENT SET title=@title, description=@description WHERE event_id=@event_id", con);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("UPDATE SIS_EVENT SET event_start=@event_start, event_end=@event_end WHERE event_id=@event_id", con);
    ...
    {
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("DELETE FROM SIS_EVENT WHERE (event_id = @event_id)", con);
        cmd.Parameters.AddWithValue("@event_id", id);
        using (con)
        {
            con.Open();
    ...
    public static int addEvent(CalendarEvent cevent)
    {
        ...
        //insert
        SqlConnection con = new SqlConnection(connectionString);
        SqlCommand cmd = new SqlCommand("INSERT INTO SIS_EVENT(title, description, event_start, event_end) VALUES(@title, @description, @event_start, @event_end)", con);
        ...
        using (con)
        {
            ...
            cmd = new SqlCommand("SELECT max(event_id) FROM SIS_EVENT where title=@title AND description=@description AND event_start=@event_start AND event_end=@event_end", con);
            cmd.Parameters.AddWithValue("@title", cevent.title);
            cmd.Parameters.AddWithValue("@description", cevent.description);
            cmd.Parameters.AddWithValue("@event_start", cevent.start);
            cmd.Parameters.AddWithValue("@event_end", cevent.end);
